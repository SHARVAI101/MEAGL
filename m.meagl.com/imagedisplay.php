...
if($numberOfFlags<5 && $flaggedByCurrentUser==false){
	...
	*/
	...
	echo 	'<p  align="right" style="font-size:17px;" class="category">Category: <a href="#" style="color:#9A12B3">'.$memecategory.'</a>
	...
	//echo '</div>';
			...
			echo '<div class="posted-by" style="right:5px"><h1 align="right" style="font-size:17px">by: <a href="userprofile.php?id='.$uploaderId.'" style="color:#9A12B3;">'.$uploader.'</a></h1></div>';
		...
		<div class="flag-message flag-message<?php echo $imageId; ?>" id="flag-message<?php echo $imageId; ?>" style="display:none">	
			...
			<form class="flag-form" name="<?php echo $imageId; ?>" method="POST" action="">
				...
				<input type="radio" name="downvote" value="other"> Other <br><br> 
		...
		<div class="right-container">
			<div class="posters-plate">
				<img id="infobox-dedi-page-dp" src="../<?php echo $profilePictureLocation; ?>" >
				<?php
				/*
				<h1 class="username" style="top:0px;left:180px;"><a href="userprofile.php?id=<?php echo $uploaderId; ?>"><?php echo $uploader; ?></a></h1>
   				...
   				*/
   				?>
   				<?php
   				echo "<h2 class='username' style='margin-top:10px'>".htmlentities($uploader)."</h2>";
   				echo '<div id="user-subs-div">';
				echo '  <p class="info-subscribers">Subscribers:</p>
					 	<p id="'.$subscribeLabel.'" name="'.$subscribeLabel.'" class="info-subscribers-number dedicated-meme-sub-btn">'.$numberofSubscribers.'</p>';
				echo '</div>';
				echo '<button type="submit" onclick="subscribeFunction(\''.$imageId.'\',\''.htmlentities($uploaderId).'\',\''.$subscribeLabel.'\',\''.htmlentities($uploader).'\');" name="subscribe'.htmlentities($uploaderId).'" class="btn" id="userprofile-subscribe-button" style="margin-bottom:10px">'.$subscribeToUserInnerHtml.'</button>';
   				?>
			</div>
			...
			<div class="posters-plate">
				...
				<img src="<?php echo $categoryImageSource; ?>" class="category-dp">
		   		...
		   		*/
		   		?>  
		   		<img id="infobox-dedi-page-dp" src="<?php echo $categoryImageSource; ?>" >
		   		<?php
   				echo "<h2 class='username' style='margin-top:10px'>Category:<span style='color:#b522a8'> ".htmlentities($memecategory)."</span></h2>";
   				echo '<div id="categ-subs-div">';
				echo '  <p class="info-subscribers">Subscribers:</p>
					 	<p id="'.$memeCategorySubscribersLabel.'" name="'.$memeCategorySubscribersLabel.'" class="info-subscribers-number meme-categ-sub-btn">'.$memeCategorySubscribers.'</p>';
				echo '</div>';
				echo '<button type="submit" onclick="memeCategorySubscribeFunction(\''.$imageId.'\',\''.$memecategory.'\',\''.$memeCategorySubscribersLabel.'\');" name="subscribecategory'.$memecategory.'" class="btn" id="userprofile-subscribe-button" style="margin-bottom:10px">'.$subscribeToMemeCategoryInnerHtml.'</button>';
   				?>	
   			</div>
			...
			*/
			?>
			<div class="sharing-buttons" style="margin-top:0px;margin-left:2.5vw;width:50vw">
	...
	?>	
		<div class="comment-info-box">
			<div class="heading">
	            <p class="heading-part2 comments" id="<?php echo $numberOfCommentsLabel; ?>"><?php echo htmlentities($numberOfComments); ?> Comments</p>
	        </div>
			<div class="submission" id="submission-dedicated">
				<form method="post" class="commentform" id="commentform<?php echo htmlentities($imageId); ?>" action="" onsubmit="">
					<input type="hidden" id="imageId" value="<?php echo htmlentities($imageId); ?>">
				  	...
				  	<textarea class="comment-input form-control" rows="1" name="comment" id="comment" placeholder="Write comment" style="font-size:17px"></textarea>
				  	<?php
				 		if(isset($_SESSION['id'])){  
					...
					if(isset($_SESSION['id']) && $_SESSION['id']==$uploaderId){
						...
						while($row1=mysqli_fetch_assoc($result1)){
							...
							if($row1['groupId']!=0){
								...
								echo '<p style="margin-bottom:5px;width:250px"><a href="imagedisplay.php?id='.htmlentities($imageId).'&world=0&uid=0&gid='.htmlentities($groupId).'"   class="shared">'.$groupname.': My Group</a></p>';
								echo '<hr>';
							}else{					
								...
								echo '<p style="margin-bottom:5px;width:250px"><a href="imagedisplay.php?id='.htmlentities($imageId).'&world=0&uid='.htmlentities($userId).'&gid=0"  class="shared">'.$username.': My Friend</a></p>';
								echo '<hr>';
							}
						}
					}
					...
					while($row1=mysqli_fetch_assoc($result1)){
						...
						if($row1['groupId']!=0){
							...
							if(mysqli_num_rows($result101)!=0)
							{
								...
								$row2=mysqli_fetch_assoc($result2);
								$groupname=$row2['groupname'];
								echo '<p style="margin-bottom:5px;"><a href="imagedisplay.php?id='.htmlentities($imageId).'&world=0&uid=0&gid='.htmlentities($groupId).'"   class="shared">'.$groupname.': Your Group</a></p>';
								echo '<hr>';
							}
						}else{					
							//means this is a user share
							...
							{									
								$username=$uploader;
								echo '<p style="margin-bottom:5px;"><a href="imagedisplay.php?id='.htmlentities($imageId).'&world=0&uid='.htmlentities($userId).'&gid=0"  class="shared">You: Personal Sharing</a></p>';
								echo '<hr>';
							}
