...
if(isset($_POST['friend_name']))
{
	...
	if(isset($un)){
		foreach($un as $u){
			$fid=mysqli_real_escape_string($conn,$u['id']);
			$fname=mysqli_real_escape_string($conn,$u['username']);
			...
			if(mysqli_num_rows($result)!=0)
			{
				...
				if((!isset($row00['invitationStatus'])) || (isset($row00['invitationStatus']) && $row00['invitationStatus']!=0 && $row00['invitationStatus']!=1))
				{
					...
					$suggestion.= '<label class="checkbox" style="margin-top:5px;margin-left:10px"><input type="checkbox" class="groupFriendInviteCheckbox" name="new_group_user_ids[]" value="'.htmlentities($fid).'">'.htmlentities($fname).'</label><hr id="friend_hr'.htmlentities($fid).'">';
