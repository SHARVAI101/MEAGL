		...
		if($_FILES['answerReplyMeme']['size']!=0) {
			...
			if($file_size<6000000){//max file size is 6MB
				...
				if($ext == "jpg" || $ext == "png" || $ext == "jpeg" || $ext == "gif" ){
					...
					if(isset($_POST['answerreply']) && $_POST['answerreply']!=""){
					...
					echo '
                        <div class="answerreply_div reply-comment" id="'.htmlentities($answerreplydivname).'">
                          ...
                          <span class="replyUsername"><a class="answerreplyUsername"><span id="dedicated-username" style="font-size:19px">'.htmlentities($username).'</span></a></span>
                          <span class="answerreply reply" id="answerreply'.htmlentities($answerreplyId).'" contenteditable="false" style="font-size:17px">'.htmlentities($answerreply).'</span></p>
                          <br>';
                    
                    echo '<img src="'.$filepath.'" style="max-width:68vw"><br><br>
                            ';
                    echo '
                          <span class="answerreplyDatetime replyDatetime">'.getTime($datetime).'</span>
                          <span class="answerreplyLikes comment-reply-info" id="answerreplylikes'.htmlentities($answerreplyId).'" style="margin-left:0px;font-size:15px">Likes: 0</span>';
                                    
                    echo '<a type="button" class="likeanswerreplybutton reply-like-link" onclick="answerreplylikeFunction(\''.htmlentities($answerreplyId).'\',\''.htmlentities($answerreplylikesLabel).'\');" id="answerreplylike'.htmlentities($answerreplyId).'"  name="'.htmlentities($answerreplyId).'" style="margin-left:5px;font-size:15px">Like</a>
                          ';
                                    
                    echo '<div><a class="display-edit"><span class="glyphicon glyphicon-chevron-down"></span></a></div>
                    	  <div class="edit-panel"><button type="submit" class="deleteanswerreplybutton" id="'.htmlentities($answerId).'" name="'.htmlentities($answerreplyId).'">Delete reply</button></div>';
                    
                    echo '<p id="answerreplyLikeError'.htmlentities($answerreplyId).'" class="error-message"></p>';                       
                    echo '</div>';  
					exit;
				}
				else{
			...
			if(isset($_POST['answerreply']) && $_POST['answerreply']!=""){
                ...
                echo '
                	<div><a class="display-edit"><span class="glyphicon glyphicon-chevron-down"></span></a></div>
                	<div class="edit-panel"><button type="submit" class="deleteanswerreplybutton" id="'.htmlentities($answerId).'" name="'.htmlentities($answerreplyId).'">Delete reply</button></div>';
