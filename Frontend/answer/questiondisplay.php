...
if($numberOfFlags<3){
    ...
    echo '<div class="dedicated-question">';
     if(isset($_SESSION['id'])){
            ...
            echo '<div class="fame-box">
                    <h5 class="answers-count dedicated-answers-count">'.htmlentities($numberOfAnswers).' answers</h5>
    ...
    <form method="post" action="" enctype="multipart/form-data" id="answerform<?php echo htmlentities($questionId); ?>" name="answerform" style="margin-top:85px;margin-left:35px;width:600px;border-top:solid 1px #ccc;border-bottom:solid 1px #ccc">
        <div class="answer-form">
            ...
            ?>
            <h4 class="add-image-header">Add Image: </h4>
    ...
    <div class="allanswers" id="allanswers">
        ...
        { 
            ...
            if($answerForQuestionId==$questionId){
                ...
                $answerId=$row['id'];
                $answerByUserId=$row['answerByUserId'];
                ...
                <div class="answer_div comments-dedicated comments" id="answerdiv<?php echo htmlentities($answerId); ?>">
                    ...
                    echo '<form method="post" class="answerreplyform" id="answerreplyform'.htmlentities($answerId).'" action="" style="display:none">
                            <input type="hidden" id="answerId'.htmlentities($answerId).'" value="'.htmlentities($answerId).'" name="answerId">
                            <input type="hidden" id="datetime'.htmlentities($answerId).'" value="'.date('Y-m-d H:i:s').'" name="datetime">
                            <textarea id="replytext'.htmlentities($answerId).'" placeholder="Write reply" name="answerreply"></textarea><br>
                    ...
                    echo '<button class="includeExternalImageButton" id="includeExternalImageButton'.htmlentities($answerId).'" name="replytext'.htmlentities($answerId).'" style="display:none">Include external image</button>
                          <button class="includeLinkButton" id="includeLinkButton'.htmlentities($answerId).'" name="replytext'.htmlentities($answerId).'" style="display:none">Include link</button>';
                    ...
                    if($numberOfReplies>=1){
                        ...
                        echo '<button class="answerrepliestogglebutton" name="'.htmlentities($answerId).'" style="display:none">toggle all replies</button>';
                    }
                    ...
                    if($numberOfReplies>=1){
                        ...
                        { 
                            ...
                            if($replyToAnswerId==$answerId){
                                ...
                                $replyId=$row1['id'];
                                $replyByUserId=$row1['replyByUserId'];
                                ...
                                echo '
                                    <div class="answerreply_div reply-comment" id="'.htmlentities($replydivname).'" style="margin-left: 50px;">
                                      ...
                                      <span class="answerreply reply" id="answerreply'.htmlentities($replyId).'" contenteditable="false">'.htmlentities($reply).'</span></p>
                                      <br>';
                                if($answerReplyMemelocation!=''){
                                    echo '<img src="'.$answerReplyMemelocation.'" style="height:200px;width:200px;"><br>
                                        ';
                                }
                                echo '
                                      <span class="answerreplyDatetime replyDatetime">'.getTime($replyDatetime).'</span>
                                      <span class="answerreplyLikes comment-reply-info" id="answerreplylikes'.htmlentities($replyId).'">Likes:'.$replyLikes.'</span>';
                                    
                                echo '<a type="button" class="likeanswerreplybutton reply-like-link" onclick="answerreplylikeFunction(\''.htmlentities($replyId).'\',\''.htmlentities($answerreplylikesLabel).'\');" id="answerreplylike'.htmlentities($replyId).'"  name="'.htmlentities($replyId).'">'.$answerreplylikesinnerhtml.'</a>
                                    ...
                                    if(isset($_SESSION['id'])){
                                        if($replyByUserId==$_SESSION['id']){//if the reply has been posted by the same user who is viewing it, he'll get theoption to delete the reply
                                            echo '<button type="submit" class="deleteanswerreplybutton" id="'.htmlentities($answerId).'" name="'.htmlentities($replyId).'">Delete reply</button>';
