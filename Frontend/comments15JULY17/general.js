    ...
    $(".load_more_memes_form").submit(function(e){ 
        ...
        ({
            ...
            success: function (response) 
            {
                ...
                }else if(formtype=="index_page_shuffled_loadmore_form" || formtype=="savage_page_shuffle_loadmore_form"){
                    document.getElementById("shuffled_content_div").innerHTML+=final_response;
                }
                else if(formtype=="index_page_trending_loadmore_form"){
                    document.getElementById("trending_content_div").innerHTML+=final_response;
                }
                else if(formtype=="index_page_fresh_loadmore_form" || formtype=="savage_page_fresh_loadmore_form"){
        ...
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            ...
            if(Ajaxresponse=="login error"){
                ...
                for(i = 0; i < subscribeElements.length; i += 1){
                    ...
                    subscribeElements[i].style.letterSpacing="4px";
                ...
                for(i = 0; i < subscribeElements.length; i += 1){
                    ...
                    subscribeElements[i].style.letterSpacing="1.1px";
...
$(document).ready(function(){
    $('body').on('click', '.showreplyformbutton', function() {
        if(sessionId!=''){       
            var divname="#replyform"+$(this).attr("name");//here, we get the name of the div which is being toggled..this is done by getting the value inside the "name" attribute of the show all replies button..this attribute for every button is the comment id for which the reply has been posted...we add the comment id at the end of "allreplies" and this makes the class name of the div containing all the replies for that particular comment
            //alert(divname);
            $(divname).toggle();
            ...
            $('.post-reply .submit-comment').on('click', function() {
              $(this).data('clicked', 'yes');
            });
            var isClicked = $('.myButton').data('clicked');
            var isClicked1 = $('.submit-comment').data('clicked');
            if( isClicked == 'yes') {
                alert("clicked");
               // do something
            } else {
       ...
       xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            ...
            //else if(Ajaxresponse=="undo like"){
                ...
                var elements=document.getElementsByName(likesLabel);
                for ( i = 0; i < elements.length; i += 1) {
                    elements[i].innerHTML="Likes:"+Ajaxresponse;                   
                }
        ...
        if(confirm("Do you really wish to accept the friend request from this user?")){
            ...
            ({
                ...
                success: function (response) 
                {
                    ...
                    $("#acceptfriendrequestform"+id).toggle();//hides the accept friend request form once the request has been sent
                    $("#rejectfriendrequestform"+id).toggle();//hides the reject friend request form once the request has been sent                   
...
$(document).ready(function(){
    $('body').on('click', '.showMakeNewGroupFormButton', function(){ 
        ...
        $('body').not('.make_group_div').on('click',function(){
            $('.make_group_div').hide();
        });
    });
});
...
/*$(document).ready(function(){
    $('body').on('click', '.flag-submit', function(e){ 
        e.preventDefault();
        //alert("hello gndu");
        var imageId=$(this).attr("name");
        alert(imageId);
        ...
        if(confirm("Do you really wish to flag this meme?"))
        {               
            //alert("hello b");
            //var self=this;//this done because $(this) doesnot work inside the "success" part in the ajax call
            $.ajax
            ({
                ...
                {                    
                    if(response=="success"){
                        //document.getElementById("flagimage"+imageId).innerHTML="Flagged";
                        $("#imageFlagButton"+imageId).toggle();
                        $("#meme-box"+imageId).toggle();
                        $("#flag-message"+imageId).toggle();
                    }
    ...
    if(title=="Dedicated meme page")
    {
        ...
        $('.text-reply').on('keypress',function(e){
            if(e.which==13){
                ...
                $('.text-reply').height($('.replyform').height());
                $('.replyform').height(height_s1+12);
