		...
		if($row['participantStatus']==1)
		{
			...
			<form class="sendGroupInvitesForm">
				...
				<div class="friend-list checkbox setPosition">
					...
					while($row=mysqli_fetch_assoc($result)){
						...
						if($row['relation']==1){
							...
							if((isset($row00['invitationStatus']) && $row00['invitationStatus']>1) || (!isset($row00['invitationStatus'])))
							{	
								if($counter==0)
								{
								?>
				...
				if($friends==false){
					...
					?>
					<button class="btn group_input" type="submit" style="display:none">Invite <span class="glyphicon glyphicon-chevron-right"></span></button>
					<?php
				}					
				?>
			</form>			
				...
				//while($row=mysqli_fetch_assoc($result)){
					...
					{	
						echo '<div class="meme centering meme-box'.$imageId.'"  id="meme-box'.$imageId.'">
							 	<a href="imagedisplay.php?id='.$imageId.'&world=0&uid=0&gid='.$groupId.'"><h1 class="title">'.$memetitle.'</h1></a>';
						/*if($load_more_type=="recommended")
						{
						echo 	'<h1 class="origin" style="color:#9A12B3">'.$originLabel.'</h1>';
						}*/
						echo    '<a href="imagedisplay.php?id='.$imageId.'&world=0&uid=0&gid='.$groupId.'"><img class="image" src="'.$imagelocation.'"></a>
			...
			if($viewingStatus==0)
			{
				...
				<a href="<?php echo htmlentities($nL); ?>" class="notif-link" id="notification<?php echo htmlentities($notificationId); ?>">	
				    <div class="notif">
					    ...
					    <p class="small-notif-text"><?php echo htmlentities($notification); ?></p>							
					</div>
				</a>
