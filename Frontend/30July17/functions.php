		...
		if((((isset($r_e) && $r_e==true && isset($row['language']) && $row['language']=="english")||((isset($r_h) && $r_h==true && isset($row['language']) && $row['language']=="hinglish")))|| (isset($row['uploaderId']) && isset($_SESSION['id']) && $row['uploaderId']==$_SESSION['id'])||(isset($row['askerId']) && isset($_SESSION['id']) && $row['askerId']==$_SESSION['id']))||(isset($row['type'])))
		{
			...
			if($numberOfFlags<5 && $flaggedByCurrentUser==false)
			{
				...
				echo    '<a href="imagedisplay.php?id='.$imageId.'&'.$link.'" target="_blank"><img class="image" src="'.$imagelocation.'"></a>
					 	...
					 	<h6  align="right" class="category">Category: <a href="#" style="color:#9A12B3">'.$memecategory.'</a>
				...
				if($displayFlagButton==true){
					...
					if($flaggedByCurrentUser==true){
					 ...
					 if($subscribeToUserInnerHtml=="Subscribe")
					 {
						if($numberofSubscribers>=100)
		                {
		                	...
		                	echo '<button type="submit" style="letter-spacing:2px" onclick="subscribeFunction(\''.$imageId.'\',\''.$uploaderId.'\',\''.$subscribeLabel.'\',\''.$uploader.'\');" name="subscribe'.$uploaderId.'" class="btn subscribe-button">'.$subscribeToUserInnerHtml.' <p class="subscribers" id="'.$subscribeLabel.'" name="'.$subscribeLabel.'">'.$numberofSubscribers.'</p></button>	';
		                ...
		                else if($numberofSubscribers<=10)
		                {
		                    ...
		                    echo '<button type="submit" style="letter-spacing:1.1px" onclick="subscribeFunction(\''.$imageId.'\',\''.$uploaderId.'\',\''.$subscribeLabel.'\',\''.$uploader.'\');" name="subscribe'.$uploaderId.'" class="btn subscribe-button">'.$subscribeToUserInnerHtml.'<p class="subscribers" id="'.$subscribeLabel.'" name="'.$subscribeLabel.'">'.$numberofSubscribers.'</p></button>	';
		                	echo '<p class="subscribers" id="'.$subscribeLabel.'" name="'.$subscribeLabel.'" style="width:32px;">'.$numberofSubscribers.'</p>';
		                }
		             }
					...
					echo '<div class="posted-by"><h4 align="right">-posted by:<a href="userprofile.php?id='.$uploaderId.'" style="color:#9A12B3;">'.$uploader.'</a></h4></div>
		...
		if($start_counter==$_SESSION[$session_counter_name])
		{
			...
			if($numberOfFlags<5 && $flaggedByCurrentUser==false)
			{
				...
				<div class="flag-message flag-question-message<?php echo $questionId; ?>" id="flag-question-message<?php echo $questionId; ?>" style="display:none">
					<p id="flag-p">Downvoting helps Meagl remove low quality, racist or pornographic content. You could tell us more about why you downvoted this meme:</p>
					<form class="flag-form flag-question-form" name="<?php echo $questionId; ?>" method="POST" action="">
